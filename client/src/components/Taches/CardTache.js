...
const ExpandMore = styled((props) => {
 ...
 function CardTache({Tache}) {
  ...
  return (
    <Card sx={{ maxWidth: 345 }} style={{marginTop:"20px",marginLeft:"10px",boxShadow: "12px 12px 12px 12px rgba(56, 56, 56, 0.08)"}}>
    { Tache.userId===user._id ?
      <CardHeader
        avatar={
    ...
    />}
      ...
      <CardActions disableSpacing>
        {like ? <IconButton aria-label="add to favorites" onClick={()=>toggle()} style={{color:"red"}} >
          <FavoriteIcon /></IconButton>:<IconButton aria-label="add to favorites" onClick={()=>toggle()}>
          <FavoriteIcon /></IconButton>}
        
        {Tache.userId===user._id || user.role==="admin" ?<div style={{display:"flex"}}>
        <IconButton aria-label="delete" onClick={()=>dispatch(deleteTache(Tache._id,Navigate))}>
        <DeleteIcon />
      </IconButton>
      <div>
      ...
      >
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }} style={{display:"flex",flexDirection:"column",margin:"10px"}}>
      ...
      <Button variant="contained" style={{width:"200px",marginLeft:"90px",marginTop:"20px"}}onClick={()=>dispatch(editTache(Tache._id,{titre,description,date_fin,prop,priori,etat,piece_joint},Navigate))}>Edit Task</Button>
          </Typography>
        </Box>
      </Modal>
    </div></div>:null}
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <CardContent>
          ...
          <h4>propriété:</h4>{ Tache.prop}
          </Typography>
          <Typography paragraph>
          <h4>priorité:</h4>{Tache.priori}
          </Typography>
          <Typography paragraph>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:"10px",justifyContent:"space-evenly"}}>
        <AccountCircle sx={{ color: 'action.active', mr: 1, my: 0.5 }} />
        <TextField id="input-with-sx" label="comment" variant="standard" onChange={(e)=>setBody(e.target.value)} value={body} />
        <Button variant="contained" endIcon={<SendIcon />} onClick={()=>{dispatch(addComment({body},Tache._id));dispatch(getComment());setBody("")}}> 
        Send
      </Button>

      </Box>
      <Button onClick={()=>dispatch(getComment())}>comments:</Button>
       <ListOfComment></ListOfComment> 
